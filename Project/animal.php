...
foreach ($category_list as $sp) {
    if ($_SESSION['user_id'] == $sp['user_id']) {
        $showEditColumn = true;
        break;
    }
}
        ...
        <div class="grid-row">
            <div class="grid-col">
                ...
                <div class="table-filters">
                    ...
                    <label>Family:</label>
                    <select onchange="location='?type=<?= urlencode($type) ?>&family=' + this.value + '&taxonomy=<?= urlencode($selectedTaxonomy) ?>&status=<?= urlencode($selectedStatus) ?>&name=<?= urlencode($searchName) ?>'">
                        <option value="">-All-</option>
                        <?php foreach ($family_list as $fam): ?>
                            <option value="<?= htmlspecialchars($fam) ?>" <?= $selectedFamily == $fam ? 'selected' : '' ?>><?= htmlspecialchars($fam) ?></option>
                        <?php endforeach; ?>
                    </select>

                    <label>Taxonomy:</label>
                    <select onchange="location='?type=<?= urlencode($type) ?>&family=<?= urlencode($selectedFamily) ?>&taxonomy=' + this.value + '&status=<?= urlencode($selectedStatus) ?>&name=<?= urlencode($searchName) ?>'">
                        <option value="">-All-</option>
                        <?php foreach ($taxonomy_list as $tax): ?>
                            <option value="<?= htmlspecialchars($tax) ?>" <?= $selectedTaxonomy == $tax ? 'selected' : '' ?>><?= htmlspecialchars($tax) ?></option>
                        <?php endforeach; ?>
                    </select>

                    <label>Status:</label>
                    <select onchange="location='?type=<?= urlencode($type) ?>&family=<?= urlencode($selectedFamily) ?>&taxonomy=<?= urlencode($selectedTaxonomy) ?>&status=' + this.value + '&name=<?= urlencode($searchName) ?>'">
                        <option value="">-All-</option>
                ...
                <div class="resp-scroll">
                        ...
                        <thead>
                            <tr>
                                ...
                                foreach ($columns as $col => $label):
                                    ...
                                    $url = ($col === 'edit') ? '#' : "?type=" . urlencode($type) . "&family=" . urlencode($selectedFamily) . "&taxonomy=" . urlencode($selectedTaxonomy) . "&status=" . urlencode($selectedStatus) . "&name=" . urlencode($searchName) . "&sort_by=$col&sort_order=" . ($sort_by === $col && $sort_order === 'asc' ? 'desc' : 'asc');
                                ?>
                                    <th class="sorting <?= $sorted_class ?>" data-sort-type="<?= ($col === 'category_id') ? 'int' : 'string' ?>">
                                        <?php if ($col === 'edit'): ?>
                                            <?= $label ?>
                                        <?php else: ?>
                                            <a href="<?= $url ?>"><div class="sortwrap"><?= $label ?></div></a>
                                        <?php endif; ?>
                                    </th>
                                <?php endforeach; ?>
                            </tr>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($category_list as $sp): ?>
                                <tr>
                                    ...
                                    <td><a href="species_details.php?id=<?= $sp['category_id'] ?>"><?= htmlspecialchars($sp['common_name']) ?></a></td>
                                    <td><?= htmlspecialchars($sp['status']) ?></td>
                                    <?php if ($showEditColumn): ?>
                                        <td>
                                            <?php if ($_SESSION['user_id'] == $sp['user_id']): ?>
                                                <a href="species_edit.php?id=<?= $sp['category_id'] ?>" class="edit-btn">Edit</a>
                                            <?php endif; ?>
                                        </td>
                                    <?php endif; ?>
                                </tr>
                    ...
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <a href="species_create.php?type=<?= $type ?>" class="btn-add">+ Add New <?= htmlspecialchars($type) ?></a>
                    <?php endif; ?>
                </div>
